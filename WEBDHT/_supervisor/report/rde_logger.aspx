<%@ Page Title="" Language="C#" MasterPageFile="~/_supervisor/master_page.master" AutoEventWireup="true" CodeFile="rde_logger.aspx.cs" Inherits="_supervisor_report_rde_logger" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <script>
        function btnView_Click() {
            ...
            let urlGetDataStatisticSite = `${hostname}/api/getdatastatisticlogger/?provider=${listProvider}&mark=${listMark}&model=${listModel}&status=${listStatus}&install=${listInstall}&siteStatus=${listSiteStatus}&company=${listSiteCompany}`;
        ...
        function createBody(data) {
            ...
            if (checkExistsData(data)) {
                for (let item of data) {
                    if (!isEmpty(item)) {
                        if (item != null && item != undefined) {
                            ...
                            if (item.LoggerId != null && item.LoggerId != undefined && item.LoggerId.toString().trim() != "") {
                                content += `<td>${item.LoggerId}</td>`;
                            }
                            else {
                                content += `<td></td>`;
                            }
                ...
                content = `<tr><td colspan="13">Không có dữ liệu</td></tr>`;
            }
