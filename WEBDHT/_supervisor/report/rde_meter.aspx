<%@ Page Title="" Language="C#" MasterPageFile="~/_supervisor/master_page.master" AutoEventWireup="true" CodeFile="rde_meter.aspx.cs" Inherits="_supervisor_report_rde_meter" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    <div id="main-content2">
        <div id="main-content-title">
            <h2 class="title">Thống kê đồng hồ</h2>
        </div>
        <div class="container-fluid m-t">
            <div class="row">
                ...
                <div class="col-sm-2 col-md-1">
                    <label class="label" for="chkListNation">Nước sản xuất</label>
                    <asp:CheckBoxList ID="chkListNation" runat="server" DataSourceID="NationsDataSource">
                    </asp:CheckBoxList>
                    <asp:ObjectDataSource ID="NationsDataSource" runat="server" OldValuesParameterFormatString="original_{0}" SelectMethod="GetAllNationalities" TypeName="MetersBLL"></asp:ObjectDataSource>
       ...
       <%-- <div class="container-fluid m-t">
            <rsweb:ReportViewer ID="rpt" runat="server" AsyncRendering="False"
                    ...
                    <LocalReport ReportPath="App_Data\reports\rde_meter.rdlc"
                        DisplayName="_dong_ho_tuy_chon" />
                </rsweb:ReportViewer>
        </div>--%>
    </div>
    ...
    <script>
        function btnView_Click() {
            ...
            let listNation = CheckBoxListNation();
            ...
            if (listNation.length <= 0) {
                listNation = "none";
            }
            else {
                listNation = listNation.join('|');
            }
            ...
            let urlGetDataStatisticSite = `${hostname}/api/getdatastatisticmeter/?provider=${listProvider}&nation=${listNation}&mark=${listMark}&size=${listSize}&model=${listModel}&status=${listStatus}&install=${listInstall}&siteStatus=${listSiteStatus}&company=${listSiteCompany}`;
                ...
                for (let item of data) {
                    if (!isEmpty(item)) {
                        if (item != null && item != undefined) {
                            ...
                            if (item.ReceiptDate != null && item.ReceiptDate != undefined && item.ReceiptDate.toString().trim() != "") {
                                content += `<td>${convertDate(item.ReceiptDate)}</td>`;
                            }
                            else {
                                content += `<td></td>`;
                            }
                            ...
                            if (item.Nationality != null && item.Nationality != undefined && item.Nationality.toString().trim() != "") {
        ...
        function CheckBoxListNation() {
            let varTbody = document.getElementById('<%=chkListNation.ClientID %>');
            let td = varTbody.getElementsByTagName("td")
            let tgname = varTbody.getElementsByTagName("input")
