<%@ Page Language="C#" MasterPageFile="~/DMA/master_page.master"  AutoEventWireup="true" CodeFile="ChartManagerMonthly.aspx.cs" Inherits="DMA_ChartP_ChartManagerMonthly" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    <div id="main-content2">
        <div id="main-content-title">
            <h2 class="title">Đồ Thị Tháng Theo DMA</h2>
        </div>
    ...
    <script>
        ...
        function callAPIAndDrawChart(siteid, end, start) {
            ...
            let urlGetDataChart = `${hostname}/api/getdatareportmonthlymanager/?manager=${siteid}&start=${totalSecondStart}&end=${totalSecondEnd}`;
            ...
            axios.get(urlGetDataChart).then((res) => {
                if (checkExistsData(res.data)) {
                    ...
                    titleChart.innerHTML = `Đồ thị sản lượng tháng theo DMA ${siteid}`;
        ...
        function callAPITimeAndDrawChart(siteid, end, start) {
            ...
            let now = Date.now();
            now = new Date(now);
            let date = new Date(now.getFullYear(), now.getMonth(), 1, 0, 0, 0);
            let temp = new Date(now.getFullYear(), now.getMonth(), 1,0, 0, 0);
            let startDate = new Date(temp.setMonth(temp.getMonth() - 5));
